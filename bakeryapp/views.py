...
from .models import (MyCustomUser,Bakery_category
                     ,Bakery_recipe,add_cart, our_signature_banner,
                     our_products_data,offer,review,user_comments)
...
def view_order(request, id):
    try:
        ...
        return render(request, 'view_order.html', {'data': user_orders, 'tot': tot})
    except Bakery_recipe.DoesNotExist:
        pass
...
def add_to_cart(request):
    ...
    if recipe_id and action:
        try:
            ...
            if action == 'add':
                if cart_item:  # If cart item already exists, return error
...
def list_recipe(request,id):
    ...
    return render(request, 'list_recipe.html', {'data': product_det,'x':data1})
...
@login_required(login_url='/admin_login')
def delete_our_product(request,id):
    data = our_products_data.objects.get(id = id)
    data.img.delete()
    data.delete()
    return redirect('/add_our_product')

@login_required(login_url='/admin_login')
def add_our_product(request):
    ...
    try:
        ...
        if data.user_type == 'admin':
            if request.method == 'POST':
                ...
                img = request.FILES.get('img')
                name = request.POST.get('name')
                our_products_data.objects.create(img = img, name = name) 
            return render(request, 'add_our_product.html',{'data':data1})
        else:
            return redirect('/admin_login')  
    except MyCustomUser.DoesNotExist:
        return redirect('/admin_login') 
...
def index(request):
    ...
    return render(request,'index.html',{'data':data,'data1':data1,'off':off})
    ...
    try:
        ...
        if data.user_type == 'admin':
            ...
            if request.method == 'POST':
                ...
                weight = request.POST.get('weight')
                inches = request.POST.get('inches')
                Bakery_recipe.objects.create(category = category, img1 = img1, name = name, price = price, weight = weight, inches = inches) 
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if data.user_type == 'user':
                ...
                if remember_me:
                    # Set session to expire in 30 days (30 days * 24 hours * 60 minutes * 60 seconds)
